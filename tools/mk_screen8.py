...
#   1 scr8_d1    DCQCN                      (rerun in a FRESH outdir)
#   2 scr8_d3    static cap + migration     (rerun in a FRESH outdir)
#   3 scr8_b005  D4v2 BMAX=0.005C Qt=0.025*Q_abs
#   4 scr8_b010  D4v2 BMAX=0.010C
#   5 scr8_b020  D4v2 BMAX=0.020C
#   6 scr8_b040  D4v2 BMAX=0.040C
#   7 scr8_b080  D4v2 BMAX=0.080C
...
for bm in ('0.005', '0.010', '0.020', '0.040', '0.080'):
    tag = 'scr8_b%s' % bm.replace('0.', '').rstrip()
...
# ---- assert v2 cells differ from scr8_d3 ONLY in QB2 keys + paths --------
print('')
print('=== diff check: each v2 cell vs scr8_d3 ===')
...
base = norm(load(os.path.join(B, 'scr8_d3.txt')))
bad = 0
for tag, src, over in CELLS:
    if not over:
        continue
    ...
    if oa or ob:
        ...
        print('  %-12s identical to scr8_d3 apart from QB2 keys + paths' % tag)
