...
CELLS="sx_a_dcqcn sx_a_dctcp sx_a_timely sx_a_hpcc sx_a_cbapsba sx_b_dcqcn sx_b_dctcp sx_b_cbapsba"
...
for tag in $CELLS; do
  ...
  if [ ! -f "${tag}.txt" ]; then
    echo "FAIL $tag: config missing (run mk_sens.py first)"
    exit 3
  fi
  disk_guard
  mkdir -p "${tag}_out"
