...
LOG=/work/screen7_logs
mkdir -p "$LOG"

CELLS="scr7_d1 scr7_d3 scr7_b005 scr7_b010 scr7_b020 scr7_b040 scr7_b080"
...
for tag in $CELLS; do
  ...
  if [ ! -f "${tag}.txt" ]; then
    echo "FAIL $tag: config ${tag}.txt missing (run mk_screen7.py first)"
    exit 3
  fi
  disk_guard
  mkdir -p "${tag}_out"
