...
CELLS="nb_b030 nb_b050 hp_s3 hp_s4 hp_s5"

disk_guard () {
  avail_kb=$(df --output=avail /work | tail -1 | tr -d ' ')
  if [ "$avail_kb" -lt 2097152 ]; then
    echo "DISK_GUARD_FAIL: only $((avail_kb/1024)) MB free (< 2048 MB)."
...
for tag in $CELLS; do
  ...
  if [ ! -f "${tag}.txt" ]; then
    echo "FAIL $tag: config missing (run mk_nbhp.py first)"
    exit 3
  fi
  disk_guard
  mkdir -p "${tag}_out"
