...
def main():
    ...
    updates = {
        ...
        "CBAP_INCREASE_POLICY": 1 if mode in (25, 26, 27) else 0,
        ...
        "CBAP_HANDOFF_ENABLE": int(handoff or guarded),
        ...
        "CBAP_DELEGATION_DIAGNOSTIC_FORCE_STALE_EPOCHS":
            4 if guarded and row.get("semantic_case") == "S5" else 0,
        ...
        "CBAP_HANDOFF_SUMMARY_FILE":
            "cbap_handoff_summary.csv" if mode in (25, 26, 27) else "/dev/null",
        "CBAP_HANDOFF_FLOW_FILE":
            "cbap_handoff_flow.csv" if mode in (25, 26, 27) else "/dev/null",
        "CBAP_CONTROLLER_OWNERSHIP_FILE":
            "controller_ownership.csv" if (handoff or guarded) else "/dev/null",
        "CBAP_CONTROL_MESSAGE_FILE":
            "control_message_audit.csv" if mode in (25, 26, 27) else "/dev/null",
        "CBAP_ENVELOPE_LINK_FILE":
            "envelope_link_audit.csv" if guarded else "/dev/null",
        "CBAP_ENVELOPE_FLOW_FILE":
            "envelope_flow_audit.csv" if guarded else "/dev/null",
        "CBAP_INCUMBENT_PROGRESS_FILE":
            "incumbent_progress.csv" if guarded else "/dev/null",
    ...
    input_names = ("topology.txt", "flow.txt", "trace.txt", "rounds.txt",
                   ...
                   "scenario_meta.json", "v15_input_hashes.json")
    ...
    manifest.update({
        ...
        "delegation_policy": "GUARDED" if guarded else "NONE",
        "envelope_policy": "PATH_MIN_PROPORTIONAL" if guarded else "NONE",
        "base_cc": "DCQCN" if (handoff or guarded or mode == 21) else "N/A",
