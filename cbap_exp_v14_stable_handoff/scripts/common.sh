#!/usr/bin/env bash
set -uo pipefail
V14_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REPO_ROOT="$(cd "$V14_ROOT/.." && pwd)"
...
PY
  ...
  if [[ -f "$d/completed.flag" ]] && \
      python3 "$V14_ROOT/scripts/check_outputs.py" "$d" >/dev/null 2>&1; then
    echo "SKIP valid $run_id"
    return 0
  fi
  disk_check "$output_root" || return 1
  mkdir -p "$d"
  python3 "$V14_ROOT/scripts/prepare_run.py" "$manifest" "$run_id" "$d" \
  ...
  python3 "$V14_ROOT/scripts/collect_run_metrics.py" "$d" || return 1
  python3 "$REPO_ROOT/cbap_exp/scripts/materialize_required_summaries.py" "$d" || return 1
  python3 "$V14_ROOT/scripts/check_outputs.py" --allow-no-complete-flag "$d" || return 1
